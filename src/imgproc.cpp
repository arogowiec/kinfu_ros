...
void kfusion::cuda::depthBilateralFilter(const Depth& in, Depth& out,
...
void kfusion::cuda::depthBuildPyramid(const Depth& depth, Depth& pyramid,
                                      float sigma_depth)
{
...
void kfusion::cuda::computeNormalsAndMaskDepth(const Intr& intr, Depth& depth,
                                               Normals& normals)
{
...
void kfusion::cuda::computePointNormals(const Intr& intr, const Depth& depth,
...
void kfusion::cuda::computeDists(const Depth& depth, Dists& dists,
                                 const Intr& intr)
{
  dists.create(depth.rows(), depth.cols());
  device::compute_dists(depth, dists, make_float2(intr.fx, intr.fy),
                        make_float2(intr.cx, intr.cy));
}

void kfusion::cuda::resizeDepthNormals(const Depth& depth,
                                       const Normals& normals, Depth& depth_out,
                                       Normals& normals_out)
{
...
void kfusion::cuda::renderImage(const Depth& depth, const Normals& normals,
...
void kfusion::cuda::renderImage(const Cloud& points, const Normals& normals,
                                const Intr& intr, const Vec3f& light_pose,
                                Image& image)
{
