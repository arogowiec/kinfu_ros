...
__kf_device__ int ComputeIcpHelper::find_coresp(int x, int y, float3& nd,
                                                float3& d, float3& s) const
{
  int src_z = dcurr(y, x);
  if (src_z == 0)
    return 40;
...
__kf_device__ void ComputeIcpHelper::partial_reduce(
    const float row[7], PtrStep<float>& partial_buf) const
{
...
__global__ void icp_helper_kernel(const ComputeIcpHelper helper,
                                  PtrStep<float> partial_buf)
{
  ...
  float3 n, d, s;
  int filtered = (x < helper.cols && y < helper.rows)
                     ? helper.find_coresp(x, y, n, d, s)
                     : 1;
...
__global__ void icp_final_reduce_kernel(const PtrStep<float> partial_buf,
                                        const int length, float* final_buf)
{
  ...
  float sum = 0.f;
  for (const float* t = beg + tid; t < end;
       t += ComputeIcpHelper::Policy::FINAL_REDUCE_STRIDE)
    sum += *t;
                                                   ...
                                                   float* data, cudaStream_t s)
{
  ...
  int g = Policy::TOTAL;
  icp_final_reduce_kernel<<<g, b, 0, s>>>(buffer, partials_count,
                                          buffer.ptr(Policy::TOTAL));
  cudaSafeCall(cudaGetLastError());

  cudaSafeCall(cudaMemcpyAsync(data, buffer.ptr(Policy::TOTAL),
                               Policy::TOTAL * sizeof(float),
...
void kfusion::device::ComputeIcpHelper::allocate_buffer(
    DeviceArray2D<float>& buffer, int partials_count)
{
  if (partials_count < 0)
  {
