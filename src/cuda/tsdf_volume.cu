...
struct TsdfIntegrator
{
  ...
  __kf_device__ void operator()(TsdfVolume& volume) const
  {
    ...
    float3 zstep = make_float3(vol2cam.R.data[0].z, vol2cam.R.data[1].z,
                               vol2cam.R.data[2].z) *
                   volume.voxel_size.z;

    float3 vx =
        make_float3(x * volume.voxel_size.x, y * volume.voxel_size.y, 0);
    float3 vc = vol2cam * vx;  // tranform from volume coo frame to camera one

    TsdfVolume::elem_type* vptr = volume.beg(x, y);
    for (int i = 0; i < volume.dims.z;
         ++i, vc += zstep, vptr = volume.zstep(vptr))
    {
      float2 coo = proj(vc);

      //#if defined __CUDA_ARCH__ && __CUDA_ARCH__ >= 300
      // this is actually workaround for kepler. it doesn't return 0.f for
      // texture
      // fetches for out-of-border coordinates even for cudaaddressmodeborder
      // mode
      if (coo.x < 0 || coo.y < 0 || coo.x >= dists_size.x ||
          coo.y >= dists_size.y)
        continue;
      //#endif
      ...
      if (sdf >= -volume.trunc_dist)
      {
        ...
        float tsdf_new = __fdividef(__fmaf_rn(tsdf_prev, weight_prev, tsdf),
                                    weight_prev + 1);
...
__global__ void integrate_kernel(const TsdfIntegrator integrator,
                                 TsdfVolume volume)
{
  integrator(volume);
};
...
void kfusion::device::integrate(const PtrStepSz<ushort>& dists,
...
namespace device
{
__kf_device__ void intersect(float3 ray_org, float3 ray_dir,
                             /*float3 box_min,*/ float3 box_max, float& tnear,
                             float& tfar)
{
  ...
  float3 tmin = make_float3(fminf(ttop.x, tbot.x), fminf(ttop.y, tbot.y),
                            fminf(ttop.z, tbot.z));
  float3 tmax = make_float3(fmaxf(ttop.x, tbot.x), fmaxf(ttop.y, tbot.y),
                            fmaxf(ttop.z, tbot.z));
...
__kf_device__ float interpolate(const Vol& volume, const float3& p_voxels)
{
  ...
  int3 g = make_int3(__float2int_rd(cf.x), __float2int_rd(cf.y),
                     __float2int_rd(cf.z));

  if (g.x < 0 || g.x >= volume.dims.x - 1 || g.y < 0 ||
      g.y >= volume.dims.y - 1 || g.z < 0 || g.z >= volume.dims.z - 1)
  ...
  float tsdf = 0.f;
  tsdf += unpack_tsdf(*volume(g.x + 0, g.y + 0, g.z + 0)) * (1 - a) * (1 - b) *
          (1 - c);
  tsdf +=
      unpack_tsdf(*volume(g.x + 0, g.y + 0, g.z + 1)) * (1 - a) * (1 - b) * c;
  tsdf +=
      unpack_tsdf(*volume(g.x + 0, g.y + 1, g.z + 0)) * (1 - a) * b * (1 - c);
  tsdf += unpack_tsdf(*volume(g.x + 0, g.y + 1, g.z + 1)) * (1 - a) * b * c;
  tsdf +=
      unpack_tsdf(*volume(g.x + 1, g.y + 0, g.z + 0)) * a * (1 - b) * (1 - c);
...
struct TsdfRaycaster
{
  ...
  TsdfRaycaster(const TsdfVolume& volume, const Aff3f& aff, const Mat3f& Rinv,
  ...
  __kf_device__ void operator()(PtrStepSz<ushort> depth,
                                PtrStep<Normal> normals) const
  {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  __kf_device__ void operator()(PtrStepSz<Point> points,
                                PtrStep<Normal> normals) const
  {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  __kf_device__ float3 compute_normal(const float3& p) const
  {
    ...
    float Fx1 = interpolate(
        volume, make_float3(p.x + gradient_delta.x, p.y, p.z) * voxel_size_inv);
    float Fx2 = interpolate(
        volume, make_float3(p.x - gradient_delta.x, p.y, p.z) * voxel_size_inv);
    n.x = __fdividef(Fx1 - Fx2, gradient_delta.x);

    float Fy1 = interpolate(
        volume, make_float3(p.x, p.y + gradient_delta.y, p.z) * voxel_size_inv);
    float Fy2 = interpolate(
        volume, make_float3(p.x, p.y - gradient_delta.y, p.z) * voxel_size_inv);
    n.y = __fdividef(Fy1 - Fy2, gradient_delta.y);

    float Fz1 = interpolate(
        volume, make_float3(p.x, p.y, p.z + gradient_delta.z) * voxel_size_inv);
    float Fz2 = interpolate(
        volume, make_float3(p.x, p.y, p.z - gradient_delta.z) * voxel_size_inv);
...
inline TsdfRaycaster::TsdfRaycaster(const TsdfVolume& _volume,
                                    const Aff3f& _aff, const Mat3f& _Rinv,
                                    const Reprojector& _reproj)
  : volume(_volume), aff(_aff), Rinv(_Rinv), reproj(_reproj)
{
}

__global__ void raycast_kernel(const TsdfRaycaster raycaster,
                               PtrStepSz<ushort> depth, PtrStep<Normal> normals)
{
  raycaster(depth, normals);
};

__global__ void raycast_kernel(const TsdfRaycaster raycaster,
                               PtrStepSz<Point> points, PtrStep<Normal> normals)
{
...
void kfusion::device::raycast(const TsdfVolume& volume, const Aff3f& aff,
                              const Mat3f& Rinv, const Reprojector& reproj,
                              Depth& depth, Normals& normals,
                              float raycaster_step_factor,
  ...
  __kf_device__ void operator()(PtrSz<Point> output) const
  {
    ...
    for (int z = 0; z < volume.dims.z - 1; ++z)
    {
      ...
      if (total_warp > 0)
      {
        ...
        int storage_index =
            (ftid >> Warp::LOG_WARP_SIZE) * Warp::WARP_SIZE * MAX_LOCAL_POINTS;
        ...
        for (int idx = lane; idx < total_warp;
             idx += Warp::STRIDE, pos += Warp::STRIDE)
        {
...
struct ExtractNormals
{
  ...
  __kf_device__ void operator()(float4* output) const
  {
    ...
    if (g.x > 1 && g.y > 1 && g.z > 1 && g.x < volume.dims.x - 2 &&
        g.y < volume.dims.y - 2 && g.z < volume.dims.z - 2)
    {
      float3 t;
...
size_t kfusion::device::extractCloud(const TsdfVolume& volume, const Aff3f& aff,
                                     PtrSz<Point> output)
{
...
void kfusion::device::extractNormals(const TsdfVolume& volume,
                                     ...
                                     const Aff3f& aff, const Mat3f& Rinv,
                                     float gradient_delta_factor,
                                     float4* output)
{
