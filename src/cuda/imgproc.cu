...
namespace device
{
__global__ void bilateral_kernel(const PtrStepSz<ushort> src,
                                 PtrStep<ushort> dst, const int ksz,
                                 ...
                                 const float sigma_depth2_inv_half)
{
  ...
  for (int cy = max(y - ksz / 2, 0); cy < ty; ++cy)
  {
    for (int cx = max(x - ksz / 2, 0); cx < tx; ++cx)
    {
      ...
      float weight = __expf(
          -(space2 * sigma_spatial2_inv_half + color2 * sigma_depth2_inv_half));
...
void kfusion::device::bilateralFilter(const Depth& src, Depth& dst,
                                      int kernel_size, float sigma_spatial,
                                      float sigma_depth)
{
  ...
  bilateral_kernel<<<grid, block>>>(src, dst, kernel_size,
                                    0.5f / (sigma_spatial * sigma_spatial),
...
namespace device
{
__global__ void truncate_depth_kernel(PtrStepSz<ushort> depth,
                                      ushort max_dist /*mm*/)
{
...
void kfusion::device::truncateDepth(Depth& depth, float max_dist /*meters*/)
{
  ...
  truncate_depth_kernel<<<grid, block>>>(
      depth, static_cast<ushort>(max_dist * 1000.f));
  cudaSafeCall(cudaGetLastError());
}
...
namespace device
{
__global__ void pyramid_kernel(const PtrStepSz<ushort> src,
                               PtrStepSz<ushort> dst, float sigma_depth_mult3)
{
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
...
void kfusion::device::depthPyr(const Depth& source, Depth& pyramid,
                               float sigma_depth)
{
  sigma_depth *= 1000;  // meters -> mm
...
namespace device
{
__global__ void compute_normals_kernel(const PtrStepSz<ushort> depth,
...
__global__ void mask_depth_kernel(const PtrStep<Normal> normals,
...
namespace device
{
__global__ void points_normals_kernel(const Reprojector reproj,
                                      const PtrStepSz<ushort> depth,
                                      PtrStep<Point> points,
                                      PtrStep<Normal> normals)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void kfusion::device::computePointNormals(const Reprojector& reproj,
                                          const Depth& depth, Points& points,
                                          Normals& normals)
{
  dim3 block(32, 8);
  dim3 grid(divUp(depth.cols(), block.x), divUp(depth.rows(), block.y));
...
namespace device
{
__global__ void compute_dists_kernel(const PtrStepSz<ushort> depth, Dists dists,
                                     float2 finv, float2 c)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void kfusion::device::compute_dists(const Depth& depth, Dists dists, float2 f,
                                    float2 c)
{
  dim3 block(32, 8);
  dim3 grid(divUp(depth.cols(), block.x), divUp(depth.rows(), block.y));

  compute_dists_kernel<<<grid, block>>>(depth, dists,
                                        make_float2(1.f / f.x, 1.f / f.y), c);
  cudaSafeCall(cudaGetLastError());
}
...
namespace device
{
__global__ void resize_depth_normals_kernel(const PtrStep<ushort> dsrc,
                                            const PtrStep<float4> nsrc,
                                            PtrStepSz<ushort> ddst,
                                            PtrStep<float4> ndst)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
...
void kfusion::device::resizeDepthNormals(const Depth& depth,
                                         ...
                                         Depth& depth_out, Normals& normals_out)
{
  ...
  resize_depth_normals_kernel<<<grid, block>>>(depth, normals, depth_out,
                                               normals_out);
  cudaSafeCall(cudaGetLastError());
}
...
namespace device
{
__global__ void resize_points_normals_kernel(const PtrStep<Point> vsrc,
                                             const PtrStep<Normal> nsrc,
                                             PtrStepSz<Point> vdst,
                                             PtrStep<Normal> ndst)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
                                          ...
                                          Normals& normals_out)
{
  ...
  resize_points_normals_kernel<<<grid, block>>>(points, normals, points_out,
                                                normals_out);
  cudaSafeCall(cudaGetLastError());
}
...
namespace device
{
__global__ void render_image_kernel(const PtrStep<ushort> depth,
                                    const PtrStep<Normal> normals,
                                    const Reprojector reproj,
                                    ...
                                    PtrStepSz<uchar4> dst)
{
  ...
  if (d == 0)
  {
    ...
    const float3 bgr2 =
        make_float3(236.f / 255.f, 120.f / 255.f, 120.f / 255.f);
  ...
  else
  {
    ...
    float Ix = Ax * Ka * Dx + Lx * Kd * Dx * fmax(0.f, dot(N, L)) +
               Lx * Ks * Sx * __powf(fmax(0.f, dot(R, V)), n);
...
__global__ void render_image_kernel(const PtrStep<Point> points,
                                    const PtrStep<Normal> normals,
                                    const Reprojector reproj,
                                    const float3 light_pose,
                                    PtrStepSz<uchar4> dst)
...
void kfusion::device::renderImage(const Depth& depth, const Normals& normals,
                                  const Reprojector& reproj,
                                  const float3& light_pose, Image& image)
{
  dim3 block(32, 8);
  dim3 grid(divUp(depth.cols(), block.x), divUp(depth.rows(), block.y));

  render_image_kernel<<<grid, block>>>((PtrStep<ushort>) depth, normals, reproj,
                                       light_pose, image);
  cudaSafeCall(cudaGetLastError());
}

void kfusion::device::renderImage(const Points& points, const Normals& normals,
                                  const Reprojector& reproj,
                                  const Vec3f& light_pose, Image& image)
{
  ...
  render_image_kernel<<<grid, block>>>((PtrStep<Point>) points, normals, reproj,
                                       light_pose, image);
  cudaSafeCall(cudaGetLastError());
}
...
namespace device
{
__global__ void tangent_colors_kernel(PtrStepSz<Normal> normals,
                                      PtrStep<uchar4> colors)
{
  int x = threadIdx.x + blockIdx.x * blockDim.x;
  int y = threadIdx.y + blockIdx.y * blockDim.y;
...
namespace device
{
__global__ void mergePointNormalKernel(const Point* cloud,
                                       const float8* normals,
                                       PtrSz<float12> output)
{
                                       ...
                                       const DeviceArray<float12>& output)
{
  ...
  mergePointNormalKernel<<<divUp(total, block), block>>>(cloud, normals,
                                                         output);
  cudaSafeCall(cudaGetLastError());
