...
{
  __kf_device__ static float quiet_NaN()
  {
    return __int_as_float(0x7fffffff); /*CUDART_NAN_F*/
  };
  __kf_device__ static float epsilon()
  {
...
{
  if (fabsf(c0) <
      numeric_limits<float>::epsilon())  // one root is 0 -> quadratic equation
  {
  ...
  else
  {
    ...
    if (roots.x <= 0)  // eigenval for symetric positive semi-definite matrix
                       // can not be negative! Set it to 0
      computeRoots2(c2, c1, roots);
  }
}
...
struct Eigen33
{
 public:
  ...
  __kf_device__ void compute(Mat33& tmp, Mat33& vec_tmp, Mat33& evecs,
                             float3& evals)
  {
 ...
 private:
  ...
  __kf_device__ static bool isMuchSmallerThan(float x, float y)
  {
    ...
    const float prec_sqr =
        numeric_limits<float>::epsilon() * numeric_limits<float>::epsilon();
...
struct Emulation
{
  static __kf_device__ int warp_reduce(volatile int* ptr,
