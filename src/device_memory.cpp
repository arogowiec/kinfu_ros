...
void kfusion::cuda::error(const char* error_string, const char* file,
                          const int line, const char* func)
{
  std::cout << "KinFu2 error: " << error_string << "\t" << file << ":" << line
            << std::endl;
  exit(0);
}
...
#define CV_XADD(addr, delta) \
  _InterlockedExchangeAdd((long volatile*) (addr), (delta))
#else
...
kfusion::cuda::DeviceMemory::DeviceMemory(size_t sizeBtes_arg)
  : data_(0), sizeBytes_(0), refcount_(0)
{
  create(sizeBtes_arg);
}
...
kfusion::cuda::DeviceMemory& kfusion::cuda::DeviceMemory::operator=(
    const kfusion::cuda::DeviceMemory& other_arg)
{
  if (this != &other_arg)
  {
