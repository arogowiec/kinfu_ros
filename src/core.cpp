...
inline int convertSMVer2Cores(int major, int minor)
{
  ...
  SMtoCores gpuArchCoresPerSM[] = {{0x10, 8},   {0x11, 8},   {0x12, 8},
                                   ...
                                   {0x30, 192}, {0x35, 192}, {0x50, 128},
                                   {0x52, 128}, {0x62, 128}, {-1, -1}};
...
void kf::cuda::printCudaDeviceInfo(int device)
{
  ...
  printf(", CUDA Driver Version  = %d.%d", driverVersion / 1000,
         driverVersion % 100);
...
void kf::cuda::printShortCudaDeviceInfo(int device)
{
  ...
  for (int dev = beg; dev < end; ++dev)
  {
    ...
    printf("Device %d:  \"%s\"  %.0fMb", dev, prop.name,
           (float) prop.totalGlobalMem / 1048576.0f);
    printf(
        ", sm_%d%d%s, %d cores", prop.major, prop.minor, arch_str,
...
{
  return ((double) cv::getTickCount() - start) * 1000.0 /
         cv::getTickFrequency();
}
