...
{
  return os << "([f = " << intr.fx << ", " << intr.fy << "] [cp = " << intr.cx
            << ", " << intr.cy << "])";
}
...
kfusion::device::TsdfVolume::TsdfVolume(elem_type* _data, int3 _dims,
                                        float3 _voxel_size, float _trunc_dist,
                                        int _max_weight)
  : data(_data),
    dims(_dims),
    voxel_size(_voxel_size),
    trunc_dist(_trunc_dist),
    max_weight(_max_weight)
{
}

// kfusion::device::TsdfVolume::elem_type*
// kfusionl::device::TsdfVolume::operator()(int x, int y, int z)
//{ return data + x + y*dims.x + z*dims.y*dims.x; }
//
// const kfusion::device::TsdfVolume::elem_type*
// kfusionl::device::TsdfVolume::operator() (int x, int y, int z) const
//{ return data + x + y*dims.x + z*dims.y*dims.x; }
//
// kfusion::device::TsdfVolume::elem_type* kfusionl::device::TsdfVolume::beg(int
// x, int y) const
//{ return data + x + dims.x * y; }
//
// kfusion::device::TsdfVolume::elem_type*
// kfusionl::device::TsdfVolume::zstep(elem_type *const ptr) const
...
kfusion::device::Reprojector::Reprojector(float fx, float fy, float cx,
                                          float cy)
...
// ushort2 kfusion::device::pack_tsdf(float tsdf, int weight) { throw "Not
// implemented"; return ushort2(); }
// float kfusion::device::unpack_tsdf(ushort2 value, int& weight) { throw "Not
// implemented"; return 0; }
// float kfusion::device::unpack_tsdf(ushort2 value) { throw "Not implemented";
// return 0; }
