...
{
  return kfusion::Intr(lastDepthInfo->K[0], lastDepthInfo->K[4],
                       lastDepthInfo->K[2], lastDepthInfo->K[5]);
}
...
bool RosRGBDCamera::Grab(cv::Mat& depth, cv::Mat& image)
{
  ...
  if (hasNewDepth)
  {
    cv_bridge::CvImageConstPtr cvDepth =
        cv_bridge::toCvShare(lastDepthImage, "");
    if (!cvDepth)
    {
  ...
  if (hasNewRGB)
  {
    cv_bridge::CvImageConstPtr cvColor =
        cv_bridge::toCvShare(lastRGBImage, "rgb8");
...
{
  depthSubscriber = transport.subscribeCamera<RosRGBDCamera>(
      topic, 10, &RosRGBDCamera::DepthCallback, this);
  hasNewDepth     = false;
...
{
  rgbSubscriber = transport.subscribeCamera<RosRGBDCamera>(
      topic, 10, &RosRGBDCamera::RGBCallback, this);
  hasNewRGB     = false;
  subscribedRGB = true;
}

void RosRGBDCamera::DepthCallback(
...
void RosRGBDCamera::RGBCallback(
    const sensor_msgs::ImageConstPtr& image,
    const sensor_msgs::CameraInfoConstPtr& cameraInfo)
{
  lastRGBImage = image;
