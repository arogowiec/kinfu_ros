...
KF_EXPORTS void error(const char* error_string, const char* file,
...
class KF_EXPORTS DeviceMemory2D
{
 public:
   ...
   * \param host_step_arg: stride between two consecutive rows in bytes for host
   * buffer
   * \param rows_arg: number of rows to upload
   * \param sizeBytes_arg: width of host buffer in bytes
   * */
  void upload(const void* host_ptr_arg, size_t host_step_arg, int rows_arg,
              int colsBytes_arg);
